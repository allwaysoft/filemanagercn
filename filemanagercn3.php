    ...
    if (isset($_SESSION['logged'], $auth_users[$_SESSION['logged']])) {
        ...
        <div class="path">
            <form action="" method="post" style="margin:10px;text-align:center">
                <input name="fm_usr" value="" placeholder="用户名" required>
                <input type="password" name="fm_pwd" value="" placeholder="密码" required>
...
if (isset($_GET['new'])) {
    ...
    if ($new != '' && $new != '..' && $new != '.') {
        ...
        if (fm_mkdir($path . '/' . $new, false) === true) {
            fm_set_msg(sprintf('文件夹 <b>%s</b> 已创建', fm_enc($new)));
...
if (isset($_GET['copy'], $_GET['finish'])) {
    ...
    if ($from != $dest) {
        ...
        if ($move) {
            ...
            if ($rename) {
                fm_set_msg(sprintf('移动 <b>%s</b> 到 <b>%s</b>', fm_enc($copy), fm_enc($msg_from)));
            } elseif ($rename === null) {
                fm_set_msg('文件或文件夹已经存在', 'alert');
            } else {
                fm_set_msg(sprintf('移动 <b>%s</b> 到 <b>%s</b>发生错误', fm_enc($copy), fm_enc($msg_from)), 'error');
            }
        } else {
            if (fm_rcopy($from, $dest)) {
                fm_set_msg(sprintf('复制 <b>%s</b> 到 <b>%s</b>', fm_enc($copy), fm_enc($msg_from)));
            } else {
                fm_set_msg(sprintf('复制 <b>%s</b> 到 <b>%s</b>发生错误', fm_enc($copy), fm_enc($msg_from)), 'error');
            }
        }
    } else {
        fm_set_msg('路径必须不同', 'alert');
    }
    fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
}
...
if (isset($_POST['file'], $_POST['copy_to'], $_POST['finish'])) {
    ...
    if (is_array($files) && count($files)) {
        ...
        if ($errors == 0) {
            $msg = $move ? '选定的文件和文件夹已移动' : '选定的文件和文件夹已复制';
            fm_set_msg($msg);
        } else {
            $msg = $move ? '移动项目时发生错误' : '复制项目时发生错误';
...
if (isset($_GET['ren'], $_GET['to'])) {
    ...
    if ($old != '' && $new != '') {
        if (fm_rename($path . '/' . $old, $path . '/' . $new)) {
            fm_set_msg(sprintf('重命名 <b>%s</b> 为 <b>%s</b>', fm_enc($old), fm_enc($new)));
        } else {
            fm_set_msg(sprintf('重命名 <b>%s</b> 为 <b>%s</b>发生错误', fm_enc($old), fm_enc($new)), 'error');
        }
    } else {
        fm_set_msg('名称未指定', 'error');
    }
    fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
}
...
if (isset($_POST['upl'])) {
    ...
    if ($errors == 0 && $uploads > 0) {
        fm_set_msg(sprintf('全部文件上传到 <b>%s</b>', fm_enc($path)));
    } elseif ($errors == 0 && $uploads == 0) {
        fm_set_msg('未上传任何项目', 'alert');
    } else {
        fm_set_msg(sprintf('上传文件时发生错误. 上传文件: %s', $uploads), 'error');
    }
...
if (isset($_POST['group'], $_POST['delete'])) {
    ...
    if (is_array($files) && count($files)) {
        ...
        if ($errors == 0) {
            fm_set_msg('选定文件和文件夹已删除');
        } else {
            fm_set_msg('删除项目时发生错误', 'error');
        }
    } else {
        fm_set_msg('未选择任何项目', 'alert');
    }
...
if (isset($_GET['upload'])) {
    ...
    <div class="path">
        <p><b>上传文件</b></p>
        <p class="break-word">目标文件夹: <?php echo fm_enc(FM_ROOT_PATH . '/' . FM_PATH) ?></p>
        <form action="" method="post" enctype="multipart/form-data">
            ...
            <br>
            <p>
                <button class="btn"><i class="icon-apply"></i>上传</button> &nbsp;
...
if (isset($_POST['copy'])) {
    ...
    <div class="path">
        ...
        <form action="" method="post">
            ...
            <p class="break-word">源文件夹: <?php echo fm_enc(FM_ROOT_PATH . '/' . FM_PATH) ?><br>
                <label for="inp_copy_to">目标文件夹:</label>
                <?php echo FM_ROOT_PATH ?>/<input name="copy_to" id="inp_copy_to" value="<?php echo fm_enc(FM_PATH) ?>">
            </p>
            <p><label><input type="checkbox" name="move" value="1">移动</label></p>
            <p>
...
if (isset($_GET['copy']) && !isset($_GET['finish'])) {
    ...
    <div class="path">
        <p><b>Copying</b></p>
        <p class="break-word">
            源路径: <?php echo fm_enc(FM_ROOT_PATH . '/' . $copy) ?><br>
            目标文件夹: <?php echo fm_enc(FM_ROOT_PATH . '/' . FM_PATH) ?>
        </p>
        <p>
    ...
    <div class="path">
        ...
        <p>
            ...
            if ($is_zip && $filenames !== false) {
                ...
                ?>
                <b><a href="?p=<?php echo urlencode(FM_PATH) ?>&amp;unzip=<?php echo urlencode($file) ?>"><i class="icon-apply"></i>解压</a></b> &nbsp;
                <b><a href="?p=<?php echo urlencode(FM_PATH) ?>&amp;unzip=<?php echo urlencode($file) ?>&amp;tofolder=1" title="解压到 <?php echo fm_enc($zip_name) ?>"><i class="icon-apply"></i>
                    解压到文件夹</a></b> &nbsp;
                <?php
            }
            ?>
...
<?php endif; ?>
<td>
<a title="删除" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;del=<?php echo urlencode($f) ?>" onclick="return confirm('确定要删除文件夹?');"><i class="icon-cross"></i>删除</a>
<a title="重命名" href="#" onclick="rename('<?php echo fm_enc(FM_PATH) ?>', '<?php echo fm_enc($f) ?>');return false;"><i class="icon-rename"></i>重命名</a>
<a title="复制" href="?p=&amp;copy=<?php echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>"><i class="icon-copy"></i>复制</a>
<a title="打开链接" href="<?php echo fm_enc(FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $f . '/') ?>" target="_blank"><i class="icon-chain"></i>打开链接</a>
</td></tr>
    <?php
    flush();
}
...
<tr>
<td><label><input type="checkbox" name="file[]" value="<?php echo fm_enc($f) ?>"></label></td>
<td><div class="filename"><a href="<?php echo fm_enc($filelink) ?>" title="文件信息"><i class="<?php echo $img ?>"></i> <?php echo fm_enc($f) ?></a><?php echo ($is_link ? ' &rarr; <i>' . fm_enc(readlink($path . '/' . $f)) . '</i>' : '') ?></div></td>
...
<?php endif; ?>
<td>
<a title="删除" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;del=<?php echo urlencode($f) ?>" onclick="return confirm('确定要删除文件?');"><i class="icon-cross"></i>删除</a>
<a title="重命名" href="#" onclick="rename('<?php echo fm_enc(FM_PATH) ?>', '<?php echo fm_enc($f) ?>');return false;"><i class="icon-rename"></i>重命名</a>
<a title="复制" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;copy=<?php echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>"><i class="icon-copy"></i>复制</a>
<a title="打开链接" href="<?php echo fm_enc(FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $f) ?>" target="_blank"><i class="icon-chain"></i>打开链接</a>
<a title="下载" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;dl=<?php echo urlencode($f) ?>"><i class="icon-download"></i>下载</a>
</td></tr>
    <?php
    flush();
}
...
if (empty($folders) && empty($files)) {
    ?>
<tr><td></td><td colspan="<?php echo !FM_IS_WIN ? '6' : '4' ?>"><em>文件夹为空</em></td></tr>
<?php
} else {
    ?>
<tr><td class="gray"></td><td class="gray" colspan="<?php echo !FM_IS_WIN ? '6' : '4' ?>">
总体大小: <span title="<?php printf('%s bytes', $all_files_size) ?>"><?php echo fm_get_filesize($all_files_size) ?></span>,
文件数: <?php echo $num_files ?>,
文件夹数: <?php echo $num_folders ?>
</td></tr>
<?php
}
?>
</table>
<p class="path"><a href="#" onclick="select_all();return false;"><i class="icon-checkbox"></i>全部选择</a> &nbsp;
...
<a title="上传文件" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;upload"><i class="icon-upload"></i>上传文件</a>
<a title="新建文件夹" href="#" onclick="newfolder('<?php echo fm_enc(FM_PATH) ?>');return false;"><i class="icon-folder_add"></i>新建文件夹</a>
<?php if (FM_USE_AUTH): ?><a title="退出" href="?logout=1"><i class="icon-logout"></i>退出</a><?php endif; ?>
</div>
        <?php
        $path = fm_clean_path($path);
        $root_url = "<a href='?p='><i class='icon-home' title='" . FM_ROOT_PATH . "'></i>主目录</a>";
...
<script>
function newfolder(p){var n=prompt('新文件夹名称','新建文件夹');if(n!==null&&n!==''){window.location.search='p='+encodeURIComponent(p)+'&new='+encodeURIComponent(n);}}
